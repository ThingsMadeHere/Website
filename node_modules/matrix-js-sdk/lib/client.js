...
import { OAuth2Error, isValidAuthMetadata } from "./oauth/index.js";
import { UnsupportedDelayedEventsEndpointError, UnsupportedStickyEventsEndpointError } from "./errors.js";
import { RetentionPolicyService } from "./retentionPolicy.js";
import { createRtcTransportsCachedValue } from "./rtcTransportsCachedValue.js";
import { createWellKnownCachedValue } from "./wellKnownCachedValue.js";
...
export const GET_LOGIN_TOKEN_CAPABILITY = new NamespacedValue("m.get_login_token", "org.matrix.msc3882.get_login_token");
...
export let ClientEvent = /*#__PURE__*/function (ClientEvent) {
  ...
  ClientEvent["RtcTransportsUpdated"] = "rtcTransportsUpdated";
  return ClientEvent;
}({});
...
export class MatrixClient extends TypedEventEmitter {
  constructor(opts) {
    ...
    _defineProperty(this, "cachedRtcTransports", void 0);
    _defineProperty(this, "cachedWellKnown", void 0);
  ...
  async startClient(opts) {
    ...
    if (this.clientOpts.slidingSync) {
      this.syncApi = new SlidingSyncSdk(this.clientOpts.slidingSync, this, this.clientOpts, this.buildSyncApiOptions());
    } else {
    ...
    this.cachedWellKnown.start(this.clientOpts.clientWellKnownPollPeriod !== undefined ? 1000 * this.clientOpts.clientWellKnownPollPeriod : undefined);
  ...
  clearStores(args = {}) {
    ...
    const deleteRustSdkStore = async () => {
      ...
      for (const dbname of [`${args.cryptoDatabasePrefix ?? RUST_SDK_STORE_PREFIX}::matrix-sdk-crypto`, `${args.cryptoDatabasePrefix ?? RUST_SDK_STORE_PREFIX}::matrix-sdk-crypto-meta`]) {
  ...
  async createGroupCall(roomId, type, isPtt, intent, dataChannelsEnabled, dataChannelOptions) {
    ...
    return new GroupCall(this, room, type, isPtt, intent, undefined, dataChannelsEnabled || this.isVoipWithNoMediaAllowed, dataChannelOptions, this.isVoipWithNoMediaAllowed, this.useLivekitForGroupCalls, this.livekitServiceURL).create();
  }
  ...
  async initRustCrypto(args = {}) {
    ...
    const RustCrypto = await import("./rust-crypto/index.js");
   ...
   */
  getMediaConfig(useAuthenticatedMedia = false) {
  ...
  async setAccountData(eventType, content) {
    ...
    if (!this.clientRunning) {
      this.logger.warn("Calling `setAccountData` before the client is started: `getAccountData` may return inconsistent results.");
    ...
    const updatedResolvers = Promise.withResolvers();
  ...
  async joinRoom(roomIdOrAlias, opts = {}) {
    ...
    const inviter = preJoinMembership == KnownMembership.Invite ? roomMember?.events.member?.getSender() ?? null : null;
    this.logger.debug(`joinRoom[${roomIdOrAlias}]: preJoinMembership=${preJoinMembership}, inviter=${inviter}, opts=${JSON.stringify(opts)}`);
    ...
    const res = await this.http.authedRequest(Method.Post, path, queryParams, data);
  ...
  sendCompleteEvent({
    ...
    this.logger.debug(`sendEvent of type ${type} in ${roomId} with txnId ${txnId}${delayOpts ? " (delayed event)" : ""}${queryDict ? " query params: " + JSON.stringify(queryDict) : ""}`);
  ...
  sendEventHttpRequest(event, queryOrDelayOpts, queryDict) {
    ...
    const delayOpts = queryOrDelayOpts && isSendDelayedEventRequestOpts(queryOrDelayOpts) ? queryOrDelayOpts : undefined;
  ...
  redactEvent(roomId, threadId, eventId, txnId, opts) {
    ...
    if (opts?.with_rel_types !== undefined) {
      ...
      const withRelTypesPropName = this.canSupport.get(Feature.RelationBasedRedactions) === ServerSupport.Stable ? MSC3912_RELATION_BASED_REDACTIONS_PROP.stable : MSC3912_RELATION_BASED_REDACTIONS_PROP.unstable;
  ...
  async _unstable_sendDelayedStateEvent(roomId, delayOpts, eventType, content, stateKey = "", opts = {}) {
    ...
    return this.http.authedRequest(Method.Put, path, getUnstableDelayQueryOpts(delayOpts), content, opts);
  }
  ...
  async updateScheduledDelayedEvent(delayId, action, requestOptions = {}) {
    if (!(await this.doesServerSupportUnstableFeature(UNSTABLE_MSC4140_DELAYED_EVENTS))) {
      throw new UnsupportedDelayedEventsEndpointError("Server does not support the delayed events API", `${action}ScheduledDelayedEvent`);
    }
    try {
      ...
      return await this.http.authedRequest(Method.Post, path, undefined, undefined, _objectSpread(_objectSpread({}, requestOptions), {}, {
        prefix: `${ClientPrefix.Unstable}/${UNSTABLE_MSC4140_DELAYED_EVENTS}`
      }));
    } catch (e) {
   ...
   */
  async updateScheduledDelayedEventWithActionInBody(delayId, action, requestOptions = {}) {
    ...
    try {
      return await this.http.request(Method.Post, path, undefined, data, _objectSpread(_objectSpread({}, requestOptions), {}, {
        prefix: `${ClientPrefix.Unstable}/${UNSTABLE_MSC4140_DELAYED_EVENTS}`
      }));
    } catch (e) {
      if (e instanceof MatrixError && e.errcode === "M_MISSING_TOKEN") {
        ...
        return await this.http.authedRequest(Method.Post, path, undefined, data, _objectSpread(_objectSpread({}, requestOptions), {}, {
          prefix: `${ClientPrefix.Unstable}/${UNSTABLE_MSC4140_DELAYED_EVENTS}`
        }));
      } else {
        throw e;
   ...
   * @returns Rejects: with an error response.
   */
  async sendReadReceipt(event, receiptType = ReceiptType.Read, unthreaded = false) {
    if (!event) return;
  ...
  async getUrlPreview(url, ts) {
    ...
    const resp = this.http.authedRequest(Method.Get, supportsNewEndpoint ? "/media/preview_url" : "/preview_url", {
      url,
      ts: ts.toString()
    }, undefined, {
   ...
   * @see MatrixSafetyError
   */
  async invite(roomId, userId, opts = {}) {
   ...
   * @returns Rejects: with an error response.
   */
  async forget(roomId, deleteRoom = true) {
   ...
   * @private
   */
  async getEventContext(roomId, eventId) {
  ...
  async getThreadTimeline(timelineSet, eventId) {
    ...
    if (Thread.hasServerSideSupport) {
      if (Thread.hasServerSideFwdPaginationSupport) {
        ...
        const thread = timelineSet.thread;
        const resOlder = await this.fetchRelations(timelineSet.room.roomId, thread.id, THREAD_RELATION_TYPE.name, null, {
          dir: Direction.Backward,
          from: res.start,
          recurse: recurse || undefined
        });
        ...
        while (nextBatch) {
          const resNewer = await this.fetchRelations(timelineSet.room.roomId, thread.id, THREAD_RELATION_TYPE.name, null, {
            dir: Direction.Forward,
            from: nextBatch,
            recurse: recurse || undefined
          });
  ...
  async getLatestTimeline(timelineSet) {
    ...
    if (timelineSet.threadListType !== null) {
      const res = await this.createThreadListMessagesRequest(timelineSet.room.roomId, null, 1, Direction.Backward, timelineSet.threadListType, timelineSet.getFilter());
      event = res.chunk?.[0];
    } else if (timelineSet.thread && Thread.hasServerSideSupport) {
      const recurse = this.canSupport.get(Feature.RelationsRecursion) !== ServerSupport.Unsupported;
      const res = await this.fetchRelations(timelineSet.room.roomId, timelineSet.thread.id, THREAD_RELATION_TYPE.name, null, {
        dir: Direction.Backward,
        limit: 1,
        recurse: recurse || undefined
      });
  ...
  createThreadListMessagesRequest(roomId, fromToken, limit = 30, dir = Direction.Backward, threadListType = ThreadFilterType.All, timelineFilter) {
    ...
    return this.http.authedRequest(Method.Get, path, params, undefined, opts).then(res => _objectSpread(_objectSpread({}, res), {}, {
  ...
  paginateEventTimeline(eventTimeline, opts) {
    ...
    if (isNotifTimeline) {
      ...
      promise = this.http.authedRequest(Method.Get, path, params).then(async res => {
      ...
      promise = this.fetchRelations(eventTimeline.getRoomId() ?? "", thread.id, null, null, {
        ...
        const mapper = this.getEventMapper();
        const matrixEvents = res.chunk.filter(noUnsafeEventProps).filter(getRelationsThreadFilter(thread.id)).map(mapper);
  ...
  setRoomMutePushRule(scope, roomId, mute) {
    ...
    if (!mute) {
      ...
      if (!roomPushRule) {
        ...
        const doneResolvers = Promise.withResolvers();
  ...
  async _unstable_getSharedRooms(userId) {
    ...
    do {
      ...
      const res = await this.http.authedRequest(Method.Get, path, _objectSpread(_objectSpread({}, query), tokenQuery), undefined, {
  ...
  async getVersions() {
    ...
    this.serverVersionsPromise = this.http.authedRequest(Method.Get, "/_matrix/client/versions", undefined, undefined, {
      prefix: ""
    }).catch(e => {
  ...
  refreshToken(refreshToken) {
    const performRefreshRequestWithPrefix = prefix => this.http.authedRequest(Method.Post, "/refresh", undefined, {
  ...
  async loginRequest(data) {
    return await this.http.authedRequest(Method.Post, "/login", undefined, data);
  }
  ...
  async requestLoginToken(auth) {
    ...
    return this.http.authedRequest(Method.Post, "/login/get_token", undefined,
    // no query params
    body, {
   ...
   * @returns Rejects: with an error response.
   */
  async sendStateEvent(roomId, eventType, content, stateKey = "", opts = {}) {
    const room = this.getRoom(roomId);
  ...
  async encryptStateEventIfNeeded(event, room) {
  ...
  async doesServerSupportExtendedProfiles() {
    return (await this.isVersionSupported("v1.16")) || (await this.doesServerSupportUnstableFeature(UNSTABLE_MSC4133_EXTENDED_PROFILES)) || (await this.doesServerSupportUnstableFeature(STABLE_MSC4133_EXTENDED_PROFILES));
  }
  ...
  async getExtendedProfile(userId) {
    ...
    return this.http.authedRequest(Method.Get, utils.encodeUri("/profile/$userId", {
      $userId: userId
    }), undefined, undefined, {
  ...
  async getExtendedProfileProperty(userId, key) {
    ...
    const profile = await this.http.authedRequest(Method.Get, utils.encodeUri("/profile/$userId/$key", {
      $userId: userId,
      $key: key
    }), undefined, undefined, {
      prefix: await this.getExtendedProfileRequestPrefix()
  ...
  async setExtendedProfileProperty(key, value) {
    ...
    const userId = this.getUserId();
    await this.http.authedRequest(Method.Put, utils.encodeUri("/profile/$userId/$key", {
      $userId: userId,
      $key: key
    }), undefined, {
      [key]: value
  ...
  async deleteExtendedProfileProperty(key) {
    ...
    const userId = this.getUserId();
    await this.http.authedRequest(Method.Delete, utils.encodeUri("/profile/$userId/$key", {
      $userId: userId,
      $key: key
    }), undefined, undefined, {
      prefix: await this.getExtendedProfileRequestPrefix()
  ...
  async patchExtendedProfile(profile) {
    ...
    const userId = this.getUserId();
    return this.http.authedRequest(Method.Patch, utils.encodeUri("/profile/$userId", {
      $userId: userId
    }), {}, profile, {
      prefix: await this.getExtendedProfileRequestPrefix()
    });
  ...
  async setExtendedProfile(profile) {
    ...
    const userId = this.getUserId();
    await this.http.authedRequest(Method.Put, utils.encodeUri("/profile/$userId", {
      $userId: userId
    }), {}, profile, {
      prefix: await this.getExtendedProfileRequestPrefix()
    });
   ...
   */
  async getPushers() {
    const response = await this.http.authedRequest(Method.Get, "/pushers");
  ...
  submitMsisdnToken(sid, clientSecret, msisdnToken, identityAccessToken) {
    ...
    return this.http.idServerRequest(Method.Post, "/validate/msisdn/submitToken", params, IdentityPrefix.V2, identityAccessToken ?? undefined);
  }
  ...
  async identityHashedLookup(addressPairs, identityAccessToken) {
    ...
    const response = await this.http.idServerRequest(Method.Post, "/lookup", params, IdentityPrefix.V2, identityAccessToken);
  ...
  unstableGetFileTreeSpace(roomId) {
    ...
    const purposeEvent = room.currentState.getStateEvents(UNSTABLE_MSC3088_PURPOSE.name, UNSTABLE_MSC3089_TREE_SUBTYPE.name);
